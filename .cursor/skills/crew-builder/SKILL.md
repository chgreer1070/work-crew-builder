...
**Step 1 — Map GWAs**: Review all 41 Generalized Work Activities and identify which apply to the user's role. Filter out physical/mechanical activities (items 16-23) unless specifically relevant.

**Step 2 — Generate Specific Tasks**: For each applicable GWA, generate 1-5 specific, actionable tasks contextualized to the user's role and industry.

**Step 3 — Prioritize**: Group tasks into:
- 🔴 **Critical** — Core to the role; high frequency or high impact
- 🟡 **Important** — Regular tasks that consume significant time
- 🟢 **Nice-to-have** — Occasional tasks that would benefit from assistance

**Step 4 — AI Suitability Filter**: For each task, assess:
...
|---|------|----------|-----------|
| 1 | [Task description] | 🔴 Critical | [Daily/Weekly/As-needed] |
| 2 | [Task description] | 🟡 Important | [Daily/Weekly/As-needed] |
| ... | ... | ... | ... |
...
3. I scan the `project-context/` folder for documents relevant to my tasks
   (especially my domain subfolders, e.g. research/plans/drafts/final).
4. I execute the task following crew-sop.mdc rules.
5. I save any substantial artifacts into `project-context/` in the appropriate
   subfolder (e.g. `/research`, `/plans`, `/drafts`, `/final`).
6. I update STATUS.md with my progress and results.
...
| Status entries | `[AGENT_NAME] - [TASK] - [STATUS]` | `RILEY - Weekly Report - ✅ DONE` |
...
[Dynamic section — update with current projects, deadlines, or situational context]

- **Active Project:** [Project name] — [brief description]
- **Deadline:** [Date]
...
|------|------------|----------|-----------|
| [Task 1] | [Agent Name] | 🔴 | [Freq] |
| [Task 2] | [Agent Name] | 🟡 | [Freq] |
| ... | ... | ... | ... |
...
- Start a chat with `@wcb-agent.md` and use commands:
  ...
  - `*change-autonomy` — Adjust the autonomy level
...
```
### 3.4 Project Context Folder

All crews share a common `project-context/` folder at the project root.

Usage rules:

- **Read**: Before starting a task, agents should check whether there is a
  subfolder in `project-context/` that matches their domain:

  - Strategy / planning tasks → `project-context/plans/`
  - Research / analysis tasks → `project-context/research/`
  - Writing / drafts → `project-context/drafts/`
  - Final, user-facing outputs → `project-context/final/`

- **Write**: When creating substantial artifacts (reports, plans, playbooks,
  templates), agents should:
  - Save source files into the appropriate subfolder.
  - Reference those paths in `STATUS.md` so the human can find them quickly.

- **Do not** clutter the project root with agent outputs; prefer
  `project-context/` unless the human explicitly asks for another location.


## 4. File Generation Rules

### 4.1 Generation Order
1. Individual agent `.md` files → `.cursor/rules/`
2. `AGENTS.md` → project root
3. `STATUS.md` → project root
4. `MYCREW.md` → project root
